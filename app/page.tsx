...
function ImprovementCard({ improvement }: { improvement: SectionImprovement }) {
  ...
  return <article className="improvement-card">
    <div className="improvement-card-head"><div><span className="eyebrow">{improvement.section}</span><h3>{improvement.why}</h3></div><span className={`priority-badge ${improvement.priority}`}>{improvement.priority}</span></div>
    ...
    <div className="comparison"><div className="version original-version"><span className="version-label">Original</span><p>{improvement.original}</p></div><div className="version improved-version"><span className="version-label">Improved suggestion</span><p>{improvement.diff.map((part, index) => <span key={`${part.text}-${index}`} className={part.type !== 'unchanged' ? `diff-${part.type}` : undefined} title={part.type === 'modified' ? `Replaces: ${part.original}` : undefined}>{part.text}{' '}</span>)}</p></div></div>
